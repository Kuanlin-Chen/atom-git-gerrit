...
setPushOption = (topic,reviewer,wip,codereview,verified) ->
  ...
  if (codereview)
    option = option.concat "l=Code-Review+1,"
  if (verified)
    option = option.concat "l=Verified+1,"
...
module.exports =
  ...
  commit: (message, amend) ->
    ...
    amendOpt = if amend then "--amend " else ""
  ...
  push: (branch,topic,reviewe,wip,codereview,verified)->
    option = setPushOption(topic,reviewe,wip,codereview,verified)
    cmd = "-c push.default=simple push origin HEAD:refs/for/#{branch}#{option}"
  ...
  history: ->
    return callGit "log --pretty=format:\"%h %an %s\"", parseHistory, true
